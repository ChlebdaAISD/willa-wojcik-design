    ...
    <dl className={`border-y border-charcoal/12 ${className}`}>
      {rows.map(([k, v], i) => (
        <div key={i} className={`flex items-baseline justify-between gap-6 py-3.5 ${i > 0 ? 'border-t border-charcoal/10' : ''}`}>
          <dt className="text-charcoal/70 text-[14px] leading-snug">{k}</dt>
...
// Bez kolorowego paska z lewej (ban side-stripe) — złota kreska żyje w etykiecie.
    ...
    <section className="relative bg-cream-2 py-16 md:py-20">
      <div className="max-w-[1440px] mx-auto px-6 md:px-12">
        <div className="flex items-center gap-3 mb-8">
          <span className="w-8 h-px bg-gold shrink-0" aria-hidden="true" />
          <span className="eyebrow text-charcoal/70">{title}</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {items.map((it) => (
            <Link key={it.href} href={it.href}
                  className="reveal group card-lift bg-cream rounded-sm p-6 flex items-start justify-between gap-4 border border-charcoal/12">
              <div>
                <div className="font-serif text-charcoal text-xl leading-tight">{it.title}</div>
                {it.note && <div className="text-charcoal/70 text-[13.5px] mt-1.5 leading-snug">{it.note}</div>}
              </div>
